...
public class Mannschaft implements Comparable<Mannschaft>
{
  ...
  public static void main(String[]args){
    List<Mannschaft> a1 =createTabelle();
    a1.stream().filter(m->m.getPunkte()>50).forEach(m-> System.out.println(m));
    System.out.println("alle");
    a1.stream().forEach(m ->System.out.println(m));
    System.out.println("Sortiert");
    List<Mannschaft> m1 = a1.stream().filter(m-> m.getName().startsWith("F")).sorted(Comparator.comparing(Mannschaft::getName)).toList();
    m1.stream().forEach(m-> System.out.println(m));
    System.out.println("Meiste gegentore");
    Optional<Mannschaft> meistengegtore = a1.stream().max(Comparator.comparingInt(Mannschaft::getGegentore));
    meistengegtore.ifPresent(m->System.out.println(m));
    }
